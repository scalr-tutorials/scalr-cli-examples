...
export farmid=`scalr-ctl farms create-from-template --stdin < $farm_template | jq '.data.id'`
echo $farmid
...
export serverid=`scalr-ctl farms list-servers --farmId $farmid | jq '.data[0].id'|tr -d '"'`
...
while [ "$serverstatus" != "running" ]
 do echo "Status: $serverstatus"
	export serverstatus=`scalr-ctl servers get --serverId $serverid | jq '.data.status'| tr -d '"'`
	sleep 30
done
...
export orchlogid=`scalr-ctl scripts list-orchestration-logs | jq ".data[] | select(.server.id | contains($orchserverid)).id"| sed "s/\"//g"`
